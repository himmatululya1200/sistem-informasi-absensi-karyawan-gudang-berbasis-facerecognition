...
if(isset($_GET['tanggal']) && isset($_GET['jabatan_id'])){
    ...
    header("location:./");
    exit; // tambahkan ini untuk menghentikan eksekusi kode jika tidak ada parameter yang diberikan
}

$data_tanggal = explode('-', $_GET['tanggal']);

// Periksa apakah array $data_tanggal memiliki indeks yang benar
if (count($data_tanggal) != 3) {
    echo "Format tanggal tidak valid.";
    exit; // hentikan eksekusi kode jika format tanggal tidak valid
}
...
if (!checkdate($bulan, $tanggal, $tahun)) {
    ...
    exit; // hentikan eksekusi kode jika tanggal tidak valid
}

$jumlah_tanggal_pada_bulan = cal_days_in_month(CAL_GREGORIAN, $bulan, $tahun);

echo "Jumlah tanggal pada bulan $bulan tahun $tahun adalah $jumlah_tanggal_pada_bulan.";
?>
